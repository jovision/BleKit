...
import com.jovisionapp.blekit.ble.utils.HexUtil;
...
public class SplitWriter {
    ...
    private void write() {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(22+data.length);

        int packageCount = mTotalNum;
        int packageNumber = mTotalNum-size;
        int startPackage = packageNumber == 1 ? 1:0;
        int endPackage = (size == 0) ? 1:0;
        ...
        // 报文体长度（2字节）
        // md5校验（16字节）
        buffer.put((byte) packageCount);
        buffer.put((byte) packageNumber);
        buffer.put((byte) startPackage);
        buffer.put((byte) endPackage);
